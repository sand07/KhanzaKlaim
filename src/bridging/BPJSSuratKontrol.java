...
private void ChkInputActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ChkInputActionPerformed
  isForm();                
    ...
    */
    ...
    public void SuratKontrolKlaim(String nomorrawat, String nomorrm, String nomorsep) {

       this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
        String kodedokter = Sequel.cariIsi("select reg_periksa.kd_dokter from reg_periksa where reg_periksa.no_rawat='" + nomorrawat + "'");
        String kodepoli = Sequel.cariIsi("select reg_periksa.kd_poli from reg_periksa where reg_periksa.no_rawat='" + nomorrawat + "'");
        String tglperiksa = Sequel.cariIsi("select bridging_surat_kontrol_bpjs.tgl_rencana from bridging_surat_kontrol_bpjs where bridging_surat_kontrol_bpjs.no_sep='" + nomorsep + "'");
        String nomorsurkon = Sequel.cariIsi("select bridging_sep.noskdp from bridging_sep where bridging_sep.no_sep='" + nomorsep + "'");
        ...
        System.out.println(nomorsurkon);
        if (Sequel.cariIsi("select bridging_sep.jnspelayanan from bridging_sep where bridging_sep.no_sep='" + nomorsep + "'").equals("1")) {
            param.put("exprujukan", "POST RANAP.");

        } else {
          //  if (lblRujukan.getText().equals("")) {
            //    param.put("exprujukan", Valid.SetTglIndonesia(Sequel.cariIsi("select DATE_ADD(bridging_sep.tglrujukan, INTERVAL 89 DAY) from bridging_sep where bridging_sep.no_sep='" + nomorsep + "' ")));
          //  }
        }
        ...
        Valid.MyReportqry("rptBridgingSuratKontrol2.jasper", "report", "::[ Data Surat Kontrol VClaim ]::",
                ...
                + "bridging_sep.jkel,bridging_sep.diagawal,bridging_sep.nmdiagnosaawal,bridging_surat_kontrol_bpjs.tgl_surat,bridging_surat_kontrol_bpjs.no_surat,"
                + "bridging_surat_kontrol_bpjs.tgl_rencana,bridging_surat_kontrol_bpjs.kd_dokter_bpjs,bridging_surat_kontrol_bpjs.nm_dokter_bpjs,"
                + "bridging_surat_kontrol_bpjs.kd_poli_bpjs,bridging_surat_kontrol_bpjs.nm_poli_bpjs,DATE_ADD(bridging_sep.tglrujukan, INTERVAL 89 DAY) as berlakukontrol from bridging_sep inner join bridging_surat_kontrol_bpjs "
                + "on bridging_surat_kontrol_bpjs.no_sep=bridging_sep.no_sep where bridging_surat_kontrol_bpjs.no_surat='" + nomorsurkon + "'", param);
        this.setCursor(Cursor.getDefaultCursor());
    }

    public void SuratKontrolKlaimPDF(String nomorrawat, String nomorsep, String nomorrm) {
        this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
        String kodedokter = Sequel.cariIsi("select reg_periksa.kd_dokter from reg_periksa where reg_periksa.no_rawat='" + nomorrawat + "'");
        String kodepoli = Sequel.cariIsi("select reg_periksa.kd_poli from reg_periksa where reg_periksa.no_rawat='" + nomorrawat + "'");
